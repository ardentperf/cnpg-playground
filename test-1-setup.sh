...
log "Setting up infrastructure (eu, us)..."
if "${ROOT}/scripts/setup.sh"; then pass "Infrastructure setup"; else fail "Infrastructure setup"; exit 1; fi
...
if LEGACY=true "${ROOT}/demo/setup.sh"; then pass "PostgreSQL setup"; else fail "PostgreSQL setup"; exit 1; fi
...
log "Waiting for PostgreSQL metrics to be scraped..."
...
log "Waiting for recording rules to evaluate (needs 5min of data + buffer)..."
