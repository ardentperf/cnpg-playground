...
* Make sure there is no block named `synchronous` under the `postgresql` section (confirming that synchronous replication is not enabled)
...
**Important:** With asynchronous replication, the test doesn't detect data loss on every run. In a
12-hour run on September 9 2025, there were 22 failures out of 93 iterations total. You may not see
